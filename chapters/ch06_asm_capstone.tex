...
\lstinputlisting[
    style=bash,
    caption={%
        \fname{cartridge\_build.sh} \protect\\
       Building an assembly cartridge against the board firmware. The \texttt{--entry-prefix} must match your \texttt{<prefix>\_init/update/draw} symbols.%
    }
]{scripts/ch06/cartridge_build.sh}
...
\lstinputlisting[
    style=bash,
    caption={%
        \fname{cartridge\_upload.sh} \protect\\
       Uploading a cartridge to the board over its Wi-Fi access point.%
    }
]{scripts/ch06/cartridge_upload.sh}
...
\lstinputlisting[
    style=bash,
    caption={%
        \fname{build\_and\_stage\_pong\_qemu.sh} \protect\\
       Building and staging a cartridge for the emulator. Note the QEMU firmware ELF and the \texttt{upload-qemu} subcommand.%
    }
]{scripts/ch06/build_and_stage_pong_qemu.sh}
