...
We also included a short beep sound that works with the \code{prg32_audio_beep} call.
\citep{prg32tutorialascii}.
...
### Anatomy of the Counter Program

This short program demonstrates the core design pattern of every game on this platform. 
Notice how the work is split across distinct phases: `counter_update` handles the logic 
by modifying our variable in memory, while `counter_draw` focuses purely on presentation. 
...
of console helpers. While `prg32_console_write` requires the memory address of a null-terminated 
ASCII string, `prg32_console_hex32` takes a raw 32-bit numeric value directly in \reg{a0}. It 
