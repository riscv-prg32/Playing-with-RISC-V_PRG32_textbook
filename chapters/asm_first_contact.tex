...
\begin{lstlisting}[style=bash,caption={Examples: build and upload (Wi-Fi and QEMU)}]
python3 tools/prg32_game.py build <PATH_IN.S|c> \ 
--out <PATH_OUT.prg32> \
--firmware-elf build-esp32c6/PRG32.elf \
--entry-prefix <CARTRIDGE_PREFIX> \
--name <CARTRIDGE_NAME> \

python3 tools/prg32_game.py upload <PATH_OUT.prg32> --url http://192.168.4.1
\end{lstlisting}
...
\begin{lstlisting}[style=bash,caption={Build and upload a cartridge to QEMU}]
./scripts/cartridge/build_cartridge.sh <PATH_IN.S|c> <PATH_OUT.prg32> <GAME_PREFIX>
./scripts/qemu/qemu_inject_cartridge.sh <PATH_OUT.prg32>
./scripts/qemu/launch_qemu.sh
\end{lstlisting}

These commands work on both macOS and Linux. For additional details, see
\fname{README.md} and the files in \fname{docs/} (for example \fname{docs/qemu.md}).
