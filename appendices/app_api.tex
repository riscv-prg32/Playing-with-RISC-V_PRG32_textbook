...
\code{prg32\_buzzer\_tone(hz,ms,duty)} & play a tone (event sounds only) \\
\code{prg32\_audio\_note(channel,instrument,note,volume,ms)} & play a MIDI note for a duration \\
\bottomrule
\end{tabularx}
...
For detailed instructions on build/upload options and workflows (including
upload via Wi‑Fi and staging for QEMU), see Chapter~\ref{ch:firstbuild}.
The \fname{README.md} and the \fname{docs/} folder contain supporting material
